...
#import "WLSettingsVC_Volumn.h"

@interface WLSettingsVC_Volumn ()

@property (weak, nonatomic) IBOutlet UITableView *listView;

@end

@implementation WLSettingsVC_Volumn

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    
    self.navigationItem.title = @"音量";
    ...
    for (NSInteger i = 0; i < WLItems_Volumn.count; i ++) {
        [sec_0 wl_addObj:[NSObject objWithTitle:WLItems_Volumn[i] desc:nil actionIndex:i]];
    }
...
- (void)mg_tableView:(UITableView *)tableView configCell:(UITableViewCell *)cell_ model:(NSObject *)model inSec:(NSArray *)sec atIndexPath:(NSIndexPath *)indexPath {
    ...
    BOOL selected = [model.dy_title isEqualToString:WLSettingsManager.volumn];
...
- (void)mg_tableView:(UITableView *)tableView didSelectRow:(NSObject *)row inSec:(NSArray *)sec atIndexPath:(NSIndexPath *)indexPath {
    ...
    WLSettingsManager.volumn = row.dy_title;
